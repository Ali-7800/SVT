...
class Figure:
    ...
    def _plotPoint(
        self, point, point_label, x_label="x", y_label="y", z_label="z", **kwargs
    ):
        pass

    def _plotCurve(
        self, curve, curve_label, x_label="x", y_label="y", z_label="z", **kwargs
    ):
        pass

    def _plotSurface(
        self, surface, surface_label, x_label="x", y_label="y", z_label="z", **kwargs
    ):
        pass
    ...
    def plotCurve(
        self, curve, curve_label=None, x_label="x", y_label="y", z_label="z", **kwargs
    ):
        Check.object_class(curve, Measured.Curve, "Curve")
        Check.condition(
            len(curve) == self.dimension,
            ValueError,
            "Can't plot a {0}D curve on a {1}D Figure".format(
                len(curve), self.dimension
            ),
        )
    ...
    def _check_and_match_units(curve_axis, figure_axis_unit):
        Check.condition(
            (curve_axis.unit == figure_axis_unit) > 0,
            ValueError,
            "{0} component has different unit than Figure's {1}".format(
                curve_axis, figure_axis_unit
            ),
        )
...
class MatplotlibFigure(Figure):
    def __init__(
        ...
        if self.dimension == 2:
            ...
            def plotCurvesAsShadedRegion(
                ...
                for curve in curve_list:
                    Check.condition(
                        len(curve) == 2,
                        ValueError,
                        "All curves must be 2D for plotCurvesAsShadedRegion",
                    )
                    self._check_and_match_units(curve.x, self.x_unit)
                    self._check_and_match_units(curve.y, self.y_unit)
                ...
                plt.plot(
                    minimum_curve.x.value,
                    minimum_curve.y.value,
                    color,
                    label=curves_label,
                )
                plt.plot(maximum_curve.x.value, maximum_curve.y.value, color)
                plt.fill(
                    np.append(minimum_curve.x.value, maximum_curve.x.value[::-1]),
                    np.append(minimum_curve.y.value, maximum_curve.y.value[::-1]),
                    color=color,
                    alpha=alpha,
                )
            ...
            def plotCurvesAsShadedRegion(
                self, curve_list, curves_label=None, x_label="x", y_label="y", **kwargs
            ):
                raise RuntimeError("This method can only be used for 2D Figures")

        self.plotCurvesAsShadedRegion = plotCurvesAsShadedRegion.__get__(
            self, MatplotlibFigure
        )
    ...
    def _plotCurve(
        ...
        if self.dimension == 2:
            ...
            plt.plot(
                curve.x.value, curve.y.value, curve.z.value, label=curve_label, **kwargs
            )
