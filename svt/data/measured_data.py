from svt.data.units import Unit, MiscUnit
...
class Measured:
    def __init__(self, value, unit: Union[Unit, MiscUnit]):
        if isinstance(unit, Unit):
            ...
            Check.object_class_validity(value, self.valid_values(), "value")
            self.SI_value = (
                self.misc_unit.convert_to_SI(value)
                * self.unit.alternate_prefix.multiplier
            )
            self.value = value
            self.unit.alternate_prefix.multiplier = 1.0
            self.unit.alternate_prefix.power = self.unit.alternate_prefix.SI_power
        ...
        if isinstance(other, Measured):
            if self.SI or other.SI:
                if (other.unit == self.unit) > 0:
                    m1, m2, new_unit = Unit.unify_prefixes(self.unit, other.unit)
                    return Measured(
                        m1 * self.SI_value + m2 * other.SI_value, new_unit
                    )  # this will always return the SI unit
                else:
                    raise ValueError(
                        "Measured objects must have the same unit symbols and dimensions for addition"
                    )
                ...
                if self.misc_unit == other.misc_unit:
                    ...
                    raise ValueError(
                        "Measured objects must have the same Non SI units for addition"
                    )
        return NotImplemented
        ...
        if isinstance(other, Measured):
            if self.SI or other.SI:
                if (other.unit == self.unit) > 0:
                    ...
                    raise ValueError(
                        "Measured objects must have the same unit symbols and dimensions for subtraction"
                    )
            elif not self.SI and not other.SI:
                if self.misc_unit == other.misc_unit:
                    ...
                    raise ValueError(
                        "Measured objects must have the same Non SI units for subtraction"
                    )
        return NotImplemented
    ...
    def __mul__(self, other):
        if isinstance(other, Measured):
            return Measured(
                self.SI_value * other.SI_value, self.unit * other.unit
            )  # this will always return the SI unit
    ...
    def match_unit_to(
        self, unit_to_match: Union[Unit.Collection, Unit, MiscUnit], name=None
    ):
        if isinstance(unit_to_match, Unit.Collection):
            if name is None:
                if self.unit.dimension.symbol in unit_to_match:
                    if isinstance(unit_to_match[self.unit.dimension.symbol], Unit):
                        m1, m2, new_unit = Unit.unify_prefixes_with_second_symbol(
                            self.unit, unit_to_match[self.unit.dimension.symbol]
                        )
                        temp = Measured(m1 * self.SI_value, new_unit)
                        temp = temp.convert_to(
                            unit_to_match[
                                self.unit.dimension.symbol
                            ].alternate_prefix.symbol
                        )
                        self.unit = temp.unit
                        self.SI_value = temp.SI_value
                        self.value = temp.value
                        self.shape = temp.shape
                        self.SI = (
                            True  # since addition with SI always returns an SI value
                        )
                    else:
                        temp = Measured(
                            unit_to_match[self.unit.dimension.symbol].convert_from_SI(
                                self.SI_value
                            ),
                            unit_to_match[self.unit.dimension.symbol],
                        )  # new Measured with MiscUnit
                        self.unit = temp.unit
                        self.SI_value = temp.SI_value
                        self.value = temp.value
                        self.shape = temp.shape
                ...
                Check.condition(
                    name in unit_to_match,
                    ValueError,
                    "Given name is not in Unit.Collection",
                )
                Check.condition(
                    (unit_to_match[name] == self.unit) > 0,
                    ValueError,
                    "Given name does not have units that match",
                )
                if isinstance(unit_to_match[name], Unit):
                    m1, m2, new_unit = Unit.unify_prefixes_with_second_symbol(
                        self.unit, unit_to_match[name]
                    )
                    temp = Measured(m1 * self.SI_value, new_unit)
                    ...
                    temp = Measured(
                        unit_to_match[name].convert_from_SI(self.SI_value),
                        unit_to_match[name],
                    )  # new Measured with unit to match
                    self.unit = temp.unit
                    self.SI_value = temp.SI_value
                    self.value = temp.value
                    self.shape = temp.shape
            ...
            Check.condition(
                (unit_to_match == self.unit) > 0,
                ValueError,
                "Given name does not have units that match",
            )
            m1, m2, new_unit = Unit.unify_prefixes_with_second_symbol(
                self.unit, unit_to_match
            )
            temp = Measured(m1 * self.SI_value, new_unit)
            ...
            Check.condition(
                (unit_to_match.SI_unit == self.unit) > 0,
                ValueError,
                "Given name does not have units that match",
            )
            temp = Measured(
                unit_to_match.convert_from_SI(self.SI_value), unit_to_match
            )  # new Measured with MiscUnit
            self.unit = self.unit
    ...
    def append(self, values):
        ...
        if self.SI:
            ...
            self.SI_value = np.append(
                self.SI_value, self.misc_unit.convert_to_SI(np.array(values))
            )
    ...
    class Collection:
        ...
        def save_to(self, name: str, folder="/"):
            Check.condition(
                folder[-1] == "/", ValueError, "Folder path must end with /"
            )
            if folder == "/":
                folder = ""
    ...
    class Point(Collection):
        def __init__(self, x, y, z=None) -> None:
            if z is None:
                Measured.Collection.__init__(self, x=x, y=y)
            else:
        ...
        def _shape_check(self, data):
            Check.condition(
                data.shape == 1, ValueError, "All Point components must be of shape 1"
            )
    ...
    class Curve(Collection):
        ...
        def _shape_check(self, data):
            if self.shape is None:
                Check.condition(
                    data.shape != 1,
                    ValueError,
                    "Must have more than one point for Curve",
                )
                Check.condition(
                    len(data.shape) == 1,
                    ValueError,
                    "Must be a 1D array for a Curve component",
                )
                ...
                Check.condition(
                    self.shape == data.shape,
                    ValueError,
                    "All Curve components must have the same shape",
                )
        ...
        def find_common_range(first_curve, second_curve, axis="x"):
            ...
            Check.condition(
                (first_axis.unit == second_axis.unit) > 0,
                ValueError,
                "Curve axes units must match to find common axis",
            )
            if second_axis.SI:
                first_axis += Measured(
                    0, second_axis.unit
                )  # this converts to SI if possible
            if first_axis.SI:
                second_axis += Measured(
                    0, first_axis.unit
                )  # this converts to SI if possible
            return max(min(first_axis), min(second_axis)), min(
                max(first_axis), max(second_axis)
            )

        @staticmethod
        def find_common_axis(first_curve, second_curve, axis="x"):
            range_min, range_max = Measured.Curve.find_common_range(
            ...
            common_axis = np.array(
                sorted(list(set(first_axis.value).union(set(second_axis.value))))
            )
        ...
        def interpolate_based_on_new_x(curve, new_x_axis):
            ...
            Check.condition(
                (curve.x.unit == new_x_axis.unit) > 0,
                ValueError,
                "New x axis units must match Measured.Curve x to interpolate",
            )
            if curve.x.SI:
                new_x_axis.match_unit_to(curve.x.unit)
            else:
                new_x_axis.match_unit_to(curve.x.misc_unit)
            new_y_value = interpolate.interp1d(curve.x.value, curve.y.value, axis=0)(
                new_x_axis.value
            )
            if len(curve) == 2:
                return Measured.Curve(
                    x=new_x_axis, y=Measured(new_y_value, curve.y.unit)
                )
            else:
                new_z_values = interpolate.interp1d(
                    curve.x.value, curve.z.value, axis=0
                )(new_x_axis.value)
                return Measured.Curve(
                    x=new_x_axis,
                    y=Measured(new_y_value, curve.y.unit),
                    z=Measured(new_z_values, curve.z.unit),
                )
        ...
        def min(first_curve, second_curve):
            ...
            if (len(first_curve) == 2) and (len(second_curve) == 2):
                new_x_axis = Measured.Curve.find_common_axis(
                    first_curve, second_curve, "x"
                )
                new_first_curve = Measured.Curve.interpolate_based_on_new_x(
                    first_curve, new_x_axis
                )
                new_second_curve = Measured.Curve.interpolate_based_on_new_x(
                    second_curve, new_x_axis
                )
                new_second_curve.y += Measured(
                    0, first_curve.y.unit
                )  # this converts to SI if possible
                new_first_curve.y += Measured(
                    0, second_curve.y.unit
                )  # this converts to SI if possible
                new_y_values = np.zeros_like(new_x_axis.value)
                new_y_values[new_first_curve.y <= new_second_curve.y] = (
                    new_first_curve.y.value[new_first_curve.y <= new_second_curve.y]
                )
                new_y_values[new_first_curve.y >= new_second_curve.y] = (
                    new_second_curve.y.value[new_first_curve.y >= new_second_curve.y]
                )
                return Measured.Curve(
                    x=new_x_axis, y=Measured(new_y_values, new_first_curve.y.unit)
                )
            else:
        ...
        def _shape_check(self, data):
            if self.shape == None:
                Check.condition(
                    data.shape != 1,
                    ValueError,
                    "Must have more than one point for Surface",
                )
                self.shape == data.shape
            else:
                Check.condition(
                    self.shape == data.shape,
                    ValueError,
                    "All Surface components must have the same shape",
                )
