...
class Unit:
    ...
    def __init__(
        self, symbol, dimension, prefix, alternate_symbol=None, alternate_prefix=None
    ) -> None:
    ...
    def plot_symbol(self):
        return "${0}$".format(
            self.alternate_prefix.__str__()
            + self.alternate_symbol._symbol_with_powers()
        )
    ...
    def _update_prefix(self, new_prefix):
        if isinstance(new_prefix, Unit.Prefix):
            ...
            return multiplier, Unit(
                ...
                self.dimension,
                new_prefix,
                self.alternate_symbol,
                new_prefix / self.prefix_ratio,
            )
        NotImplemented
    ...
    def convert_prefix_to(self, prefix: str):
        return Unit(
            self.symbol,
            self.dimension,
            self.alternate_prefix.convert_to(prefix) * self.prefix_ratio,
            self.alternate_symbol,
            self.alternate_prefix.convert_to(prefix),
        )
    ...
    class Symbol:
        def __init__(
            ...
            self.symbol_with_powers = Unit._write_quotient_with_powers(
        ...
        def __pow__(self, exponent):
            if isinstance(exponent, int):
                return Unit.Symbol(
                    exponent * self.numerator, exponent * self.denominator
                )
            return NotImplemented
    ...
    class Dimension(Symbol):
        def __init__(
            ...
            for dimension in numerator:
                Check.validity(
                    dimension, "Numerator dimensions", self.valid_dimensions()
                )
            for dimension in denominator:
                Check.validity(
                    dimension, "Denominator dimensions", self.valid_dimensions()
                )
    ...
    class Prefix:
        ...
        def __mul__(self, other):
            if isinstance(other, Unit.Prefix):
                ...
                new_symbol = self.SI_prefixes()[
                    self._find_closest_SI_power_index(new_power)
                ]
    ...
    class Collection:
        ...
        def append(self, *args, **kwargs):
            ...
            for argument in args:
                ...
                if isinstance(argument, Unit):
                    ...
                    self.symbol_dict[argument.SI_unit.dimension.symbol] = (
                        argument.symbol.__str__()
                    )
        ...
        def __setitem__(self, key, value):
            ...
            Check.object_class(
                ...
                (Unit, MiscUnit),
                "value",
                "To change a key in a Unit.Collection, the value given must be a Unit or a MiscUnit",
            )
            self.dict[key] = value
